...
<div x-data="mentorDirectory(@js($this->allMentors))">
    <div class="mb-3 pb-4 border-b border-gray-200 flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-3 animate-fade-up">
        <div>
    ...
    <template x-teleport="body">
        <div class="modal-overlay" x-show="showModal" @click.self="showModal = false" x-cloak>
            <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col mx-4 sm:mx-0" style="max-height: 90vh;">
                <template x-if="selectedMentor">
                    <div>
                        {{-- Modal Header --}}
                        <div class="flex-shrink-0 flex items-start gap-4 p-4 sm:p-6 bg-[#1a3c2f]">
                            <div class="w-20 h-20 sm:w-36 sm:h-36 rounded-2xl overflow-hidden flex-shrink-0 border-2 border-white/20 shadow-lg bg-gray-200">
