...
<div onclick="openFbTotalModal()" class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-blue-600 flex items-center gap-3 lg:gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
...
<div onclick="openFbSessionsModal()" class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-yellow-500 flex items-center gap-3 lg:gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
    <div class="text-2xl flex-shrink-0">
...
<div onclick="openFbAvgModal()" class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-green-600 flex items-center gap-3 lg:gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
    <div class="text-2xl flex-shrink-0">
...
<td class="px-5 py-3 align-middle col-rating">
    ...
    {{-- Feedback Stat Modals --}}
<div id="fbTotalModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);z-index:1000;align-items:center;justify-content:center;padding:24px;">
    <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        <div class="flex items-center gap-4 px-6 py-5 border-b border-gray-100">
            <div class="w-11 h-11 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xl flex-shrink-0"><i class="fa-solid fa-chart-bar"></i></div>
            <div class="flex-1"><h2 class="text-lg font-extrabold text-slate-800">Total Responses</h2><p class="text-xs text-slate-400" id="fbTotalModalCount"></p></div>
        </div>
        <div class="px-6 py-4 max-h-80 overflow-y-auto" id="fbTotalModalBody"></div>
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-100">
            <button onclick="document.getElementById('fbTotalModal').style.display='none'" class="w-full py-2.5 text-sm font-bold text-white bg-red-900 hover:bg-red-800 rounded-xl transition">Close</button>
        </div>
    </div>
</div>

<div id="fbSessionsModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);z-index:1000;align-items:center;justify-content:center;padding:24px;">
    <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        <div class="flex items-center gap-4 px-6 py-5 border-b border-gray-100">
            <div class="w-11 h-11 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center text-xl flex-shrink-0"><i class="fa-solid fa-chalkboard-user"></i></div>
            <div class="flex-1"><h2 class="text-lg font-extrabold text-slate-800">Total Sessions</h2><p class="text-xs text-slate-400">Completed sessions that received feedback</p></div>
        </div>
        <div class="px-6 py-4 max-h-80 overflow-y-auto" id="fbSessionsModalBody"></div>
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-100">
            <button onclick="document.getElementById('fbSessionsModal').style.display='none'" class="w-full py-2.5 text-sm font-bold text-white bg-red-900 hover:bg-red-800 rounded-xl transition">Close</button>
        </div>
    </div>
</div>

<div id="fbAvgModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);z-index:1000;align-items:center;justify-content:center;padding:24px;">
    <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        <div class="flex items-center gap-4 px-6 py-5 border-b border-gray-100">
            <div class="w-11 h-11 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xl flex-shrink-0"><i class="fa-solid fa-star"></i></div>
            <div class="flex-1"><h2 class="text-lg font-extrabold text-slate-800">Average Rating</h2><p class="text-xs text-slate-400" id="fbAvgModalCount"></p></div>
        </div>
        <div class="px-6 py-4 max-h-80 overflow-y-auto" id="fbAvgModalBody"></div>
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-100">
            <button onclick="document.getElementById('fbAvgModal').style.display='none'" class="w-full py-2.5 text-sm font-bold text-white bg-red-900 hover:bg-red-800 rounded-xl transition">Close</button>
        </div>
    </div>
</div>
</div>
<script>
const _fbData = @json($this->feedbacks);

function _fbRow(item) {
    const ratingColor = { 'rating-excellent':'text-green-600', 'rating-good':'text-blue-600', 'rating-average':'text-yellow-500', 'rating-poor':'text-red-500' }[item.avgClass] ?? 'text-gray-400';
    const line1 = `${item.subject ?? '—'} — ${item.topic ?? '—'}`;
    const line2 = `${item.mentor_name ?? '—'} • ${item.date_formatted ?? '—'}`;
    return `<div class="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
        <div class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-[10px] font-bold flex-shrink-0">
            ${(item.mentor_name ?? '?').slice(0,2).toUpperCase()}
        </div>
        <div class="flex-1 min-w-0">
            <p class="text-xs font-bold text-slate-700 truncate" onmouseenter="this.title=this.scrollWidth>this.clientWidth?'${line1.replace(/'/g,'&#39;')}':'';">${line1}</p>
            <p class="text-[10px] text-gray-400 truncate" onmouseenter="this.title=this.scrollWidth>this.clientWidth?'${line2.replace(/'/g,'&#39;')}':'';">${line2}</p>
        </div>
        <div class="flex-shrink-0">
            ${item.avg !== null
                ? `<span class="${ratingColor} font-bold text-[10px] bg-gray-50 px-2 py-1 rounded border border-current opacity-80" style="font-size:10px;line-height:1;white-space:nowrap;">${item.avg} / 5 - ${item.avgLabel}</span>`
                : `<span class="text-xs text-gray-300 italic">No score</span>`}
        </div>
    </div>`;
}

function openFbTotalModal() {
    const items = [..._fbData];
    document.getElementById('fbTotalModalCount').innerText = `${items.length} response${items.length !== 1 ? 's' : ''}`;

    const groups = { Excellent: 0, Good: 0, Average: 0, Poor: 0, 'No Score': 0 };
    items.forEach(i => {
        if (i.avg === null) groups['No Score']++;
        else if (i.avg >= 4.5) groups['Excellent']++;
        else if (i.avg >= 3.5) groups['Good']++;
        else if (i.avg >= 2.5) groups['Average']++;
        else groups['Poor']++;
    });

    const colors = { Excellent: 'text-green-600', Good: 'text-blue-600', Average: 'text-yellow-500', Poor: 'text-red-500', 'No Score': 'text-gray-400' };
    const bars = { Excellent: 'bg-green-500', Good: 'bg-blue-500', Average: 'bg-yellow-400', Poor: 'bg-red-400', 'No Score': 'bg-gray-300' };
    const total = items.length || 1;

    document.getElementById('fbTotalModalBody').innerHTML = !items.length
        ? `<p class="text-xs text-gray-400 italic py-4 text-center">No feedback yet.</p>`
        : Object.entries(groups).map(([label, count]) => `
            <div class="py-2.5 border-b border-gray-50 last:border-0">
                <div class="flex items-center justify-between mb-1.5">
                    <span class="text-xs font-bold ${colors[label]}">${label}</span>
                    <span class="text-xs font-black text-slate-700">${count} <span class="text-gray-400 font-normal">(${Math.round(count/total*100)}%)</span></span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5">
                    <div class="${bars[label]} h-1.5 rounded-full transition-all" style="width:${Math.round(count/total*100)}%"></div>
                </div>
            </div>`).join('');
    document.getElementById('fbTotalModal').style.display = 'flex';
}

function openFbSessionsModal() {
    const items = [..._fbData];
    const sessionCount = {{ $this->dashboardStats['sessions'] }};
    const feedbackCount = items.length;
    const mentorMap = {};
    items.forEach(i => {
        const name = i.mentor_name ?? '—';
        if (!mentorMap[name]) mentorMap[name] = { count: 0, totalAvg: 0, rated: 0 };
        mentorMap[name].count++;
        if (i.avg !== null) { mentorMap[name].totalAvg += i.avg; mentorMap[name].rated++; }
    });

    const sorted = Object.entries(mentorMap).sort((a,b) => b[1].count - a[1].count);

const summaryBlock = `<div class="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between mb-3">
        <div class="text-xs text-slate-500">Completed Sessions</div>
        <span class="text-sm font-black text-slate-800">${sessionCount}</span>
    </div>
    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between mb-4">
        <div class="text-xs text-slate-500">Total Feedback Responses</div>
        <span class="text-sm font-black text-slate-800">${feedbackCount}</span>
    </div>
    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Feedback per Mentor</p>`;

document.getElementById('fbSessionsModalBody').innerHTML = !sorted.length
        ? summaryBlock + `<p class="text-xs text-gray-400 italic py-4 text-center">No sessions with feedback yet.</p>`
        : summaryBlock + sorted.map(([name, data]) => {
            const avg = data.rated > 0 ? (data.totalAvg / data.rated).toFixed(1) : null;
            return `<div class="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-bold text-slate-700 truncate" onmouseenter="this.title=this.scrollWidth>this.clientWidth?'${name.replace(/'/g,'&#39;')}':'';">${name}</p>
                    <p class="text-[10px] text-gray-400">${data.count} feedback${data.count !== 1 ? 's' : ''}</p>
                </div>
                <div class="flex-shrink-0">
                    ${avg !== null
                        ? `<span class="text-xs font-black text-yellow-600">${avg}<span class="text-gray-400 font-normal text-[10px]"> / 5</span></span>`
                        : `<span class="text-[10px] text-gray-300 italic">No score</span>`}
                </div>
            </div>`;
        }).join('');
    document.getElementById('fbSessionsModal').style.display = 'flex';
}

function openFbAvgModal() {
    const mentorMap = {};
    _fbData.forEach(i => {
        const name = i.mentor_name ?? '—';
        if (!mentorMap[name]) mentorMap[name] = { totalAvg: 0, rated: 0 };
        if (i.avg !== null) { mentorMap[name].totalAvg += i.avg; mentorMap[name].rated++; }
    });

    const rated = Object.entries(mentorMap)
        .filter(([, d]) => d.rated > 0)
        .map(([name, d]) => ({ name, avg: d.totalAvg / d.rated, count: d.rated }))
        .sort((a,b) => b.avg - a.avg);

    const overallAvg = rated.length
        ? (rated.reduce((s,i) => s + i.avg, 0) / rated.length).toFixed(1)
        : '—';

    document.getElementById('fbAvgModalCount').innerText = `Overall: ${overallAvg} / 5.0 — ${rated.length} mentor${rated.length !== 1 ? 's' : ''}`;
    ...
    document.getElementById('fbAvgModalBody').innerHTML = !rated.length
        ? `<p class="text-xs text-gray-400 italic py-4 text-center">No rated responses yet.</p>`
        : rated.map((m, idx) => `
            <div class="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
                <div class="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black flex-shrink-0 ${idx === 0 ? 'bg-yellow-100 text-yellow-600' : idx === rated.length-1 ? 'bg-red-50 text-red-400' : 'bg-gray-100 text-gray-500'}">
                    ${idx + 1}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-bold text-slate-700 truncate" onmouseenter="this.title=this.scrollWidth>this.clientWidth?'${m.name.replace(/'/g,'&#39;')}':'';">${m.name}</p>
                    <p class="text-[10px] text-gray-400">${m.count} rated response${m.count !== 1 ? 's' : ''}</p>
                </div>
                <div class="flex-shrink-0 flex items-center gap-1.5">
                    <span class="text-xs font-black ${avgColor(m.avg)}">${m.avg.toFixed(1)}<span class="text-gray-400 font-normal text-[10px]"> / 5</span></span>
                    <span class="${avgColor(m.avg)} font-bold text-[10px] bg-gray-50 px-2 py-1 rounded border border-current opacity-80" style="font-size:10px;line-height:1;white-space:nowrap;">${avgLabel(m.avg)}</span>
                </div>
            </div>`).join('');
    document.getElementById('fbAvgModal').style.display = 'flex';
}
</script>